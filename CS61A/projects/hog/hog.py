...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    if num_rolls ==0:
        return picky_piggy(opponent_score) #chooses to play picky piggy
    else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who ==0:
            p0_num = strategy0(score0,score1)
            points0 = take_turn(p0_num, score1, dice, goal)
            score0 +=points0
            score0 += hog_pile(score0,score1)
        elif who ==1:
            p1_num = strategy1(score1,score0)
            points1=take_turn(p1_num,score0,dice,goal)
            score1 += points1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0,score1):
        ...
        if change > tmp:
            tmp = change
            print(f"{change} point(s)! That's a record gain for Player {who}!")
        return announce_highest(who,new,tmp)
    return commentary
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(10):
        ...
        for _ in range(trials_count):
            result += roll_dice(i+1,dice)
        average = result/trials_count
        if average > highest:
            highest = average
            highest_idx = i+1
    return highest_idx
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 10
    piggy = picky_piggy(opponent_score)
    if piggy >= cutoff:
        return 0
    else :
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if piggy >= cutoff:
        return 0
    elif (piggy + score) == opponent_score:
        return 0 
    else:
        return num_rolls
